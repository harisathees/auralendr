...
const LoansList: React.FC = () => {
    ...
    // 1. Fetch on Mount, Page Change, or Status Change (Immediate)
    useEffect(() => {
        fetchLoans();
    }, [page, statusFilter]);

    // 2. Fetch on Search Change (Debounced)
    useEffect(() => {
        ...
        <div className="bg-background-light dark:bg-background-dark min-h-screen text-primary-text dark:text-text-main font-display flex flex-col pb-safe">
            ...
            <header className="flex-none pt-12 pb-4 px-5 bg-background-light dark:bg-background-dark z-10 sticky top-0">
                <div className="flex items-center justify-between mb-6">
                    <h1 className="text-2xl font-bold tracking-tight text-primary-text dark:text-white">All Loans</h1>
            ...
            <main className="flex-1 px-5 pb-24 overflow-y-auto no-scrollbar">
                {loading ? (
                    ...
                    <div className="space-y-0 divide-y divide-gray-100 dark:divide-[#1f3d2e] rounded-xl shadow-sm dark:shadow-none">
                        {loans.map((loan) => (
