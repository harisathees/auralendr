...
import RepledgeBankForm from "./RepledgeBankForm";
...
const RepledgeBanks: React.FC = () => {
    ...
    const [editingBank, setEditingBank] = useState<RepledgeBank | null>(null);
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            await http.delete(`/repledge-banks/${deletingId}`);
            setBanks(banks.filter((b) => b.id !== deletingId));
    ...
    const handleOpenEdit = (bank: RepledgeBank) => {
        setEditingBank(bank);
        setIsFormOpen(true);
    };
        ...
        <div className="flex flex-col h-full relative">
            ...
            <header className="flex justify-between items-center p-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-primary-text dark:text-white">Repledge Banks</h1>
                        <p className="text-xs text-secondary-text dark:text-gray-400">Manage external banks for repledging</p>
                    </div>
                </div>
                <button
            ...
            <main className="flex-1 overflow-y-auto p-4 pb-24 flex flex-col gap-6 [&::-webkit-scrollbar]:hidden">
                ...
                <section>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-2">
                        {banks.map((bank) => (
                            <div key={bank.id} className="bg-white dark:bg-gray-900 rounded-[2rem] shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden group border border-gray-100 dark:border-gray-800">
                                {/* Visual Header */}
                                <div className="relative h-40 flex items-center justify-center bg-purple-50 dark:bg-purple-900/20">
                                    ...
                                    <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0">
                                        <button
                                            onClick={(e) => { e.stopPropagation(); handleOpenEdit(bank); }}
                                        ...
                                        <button
                                            onClick={(e) => { e.stopPropagation(); handleDeleteClick(bank.id); }}
                                ...
                                <div className="p-5 flex flex-col flex-1 bg-white dark:bg-gray-900 relative">
                                    ...
                                    <div className="mb-4">
                                        <h3 className="font-bold text-lg text-primary-text dark:text-white leading-tight line-clamp-1" title={bank.name}>{bank.name}</h3>
                                        <p className="text-xs text-gray-400 font-medium">{bank.branch || 'Main Branch'}</p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 mb-4">
                                        <div className="bg-gray-50 dark:bg-gray-800 p-2 rounded-lg text-center">
                                            ...
                                            <div className="font-bold text-purple-600 dark:text-purple-400">{bank.default_interest}%</div>
                                        </div>
                                        <div className="bg-gray-50 dark:bg-gray-800 p-2 rounded-lg text-center">
                                            <div className="text-[10px] text-gray-500 uppercase font-bold">Validity</div>
                                            <div className="font-bold text-gray-800 dark:text-white">{bank.validity_months}M</div>
                                        </div>
                                    </div>

                                    <div className="mt-auto pt-3 border-t border-gray-50 dark:border-gray-800/50 flex justify-between items-center text-xs">
                                        <span className="text-gray-400 font-medium">Post-Valid: <span className="text-gray-600 dark:text-gray-300">{bank.post_validity_interest}%</span></span>
                                        <span className="px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded text-gray-500 font-bold">{bank.payment_method || 'Cash'}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                        ...
                        {banks.length === 0 && (
                            <div className="col-span-full text-center p-12 bg-gray-50 dark:bg-gray-800/50 rounded-[2rem] border-2 border-dashed border-gray-200 dark:border-gray-700">
                                ...
                                <p className="text-gray-500 dark:text-gray-400 font-medium">No repledge banks configured yet.</p>
            ...
            <ConfirmationModal
                isOpen={isDeleteOpen}
                title="Delete Bank?"
                message="Are you sure you want to delete this bank? This might affect existing repledges."
