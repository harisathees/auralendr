...
const JewelNameForm: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEdit) {
                await http.put(`/jewel-names/${id}`, { name });
            } else {
                await http.post("/jewel-names", { name });
            }
    ...
    return (
        <div className="p-6 max-w-lg mx-auto">
            <div className="flex items-center gap-4 mb-6">
                <button
                    onClick={() => navigate("/admin/configs/jewel-names")}
                ...
                <h2 className="text-2xl font-bold text-primary-text dark:text-white">
                    {isEdit ? "Edit Jewel Name" : "Add Jewel Name"}
                </h2>
            </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-secondary-text dark:text-gray-300 mb-1">
                            Jewel Name <span className="text-red-500">*</span>
                        </label>
                        <input
                            type="text"
                            value={name}
