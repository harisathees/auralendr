...
export default function Reports() {
    ...
    const [pledges, setPledges] = useState<Pledge[]>([]); // Using Pledge[] instead of Customer[]
    ...
    const filteredPledges = useMemo(() => {
        return pledges.filter(pledge => {
            ...
            if (startDate && endDate && pledge.loan?.due_date) {
                // For overdue reports, we might filter by due date range. 
                // For annual, maybe loan date. 
                // Let's assume filter is on Due Date for now.
    ...
    const handleShareAllAsPdf = async () => {
        ...
        try {
            ...
            // Limit to first 50 to avoid browser crash if too many
    ...
    const handleShare = async (ref: React.RefObject<HTMLDivElement | null>, filename: string) => {
        ...
        try {
            // We need to capture the element. Since it might be scaled in preview, cloning is safest.
            // However, simple htmlToImage might work if we target the DOM node directly.
            // Let's try direct capture first, if it fails we clone.
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white p-3 sm:p-6 rounded-lg shadow-xl w-full max-w-5xl h-full max-h-[80vh] flex flex-col">
                        ...
                        <div className="flex-grow bg-slate-300 overflow-y-auto p-1 sm:p-8 rounded-b-lg">
                            <div className="transform origin-top flex justify-center">
                                <div className="a4-container scale-75 origin-top bg-white shadow-lg">
                                    {reportType === 'annual' ? (
                                ...
                                {filteredPledges.map(pledge => {
                                    ...
                                    return (
                                        <div key={pledge.id} className="a4-container scale-[0.6] origin-left mb-[-30%] bg-white shadow-md">
                                            {/* Scale down for preview, spacing adjusted */}
                                            {reportType === 'annual' ? (
